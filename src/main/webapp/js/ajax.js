...
function loadXMLAsynchroneously(method, request, parameters, id, template, append)
{
    ...
    var XMLDoc = null;
    xhr.onreadystatechange = function() { getXMLDocument(xhr, XMLDoc, id, template, append); };
...
function sendRequestAsynchroneously(method, request, parameters, id, template)
{
    //initialisation de l'objet XMLXhttpRequest
    var xhr = initRequete ();

    //var XMLDoc = null;
    //xhr.onreadystatechange = function() { getXMLDocument(xhr, XMLDoc, id, template); };
...
function getXMLDocument(xhr, XMLDoc, id, template)
{
    ...
    if (xhr.readyState==4) {
        ...
        if (xhr.status == 200) {
            ...
            if (xhr.responseXML != null) {
                ...
                XMLDoc = JSON.parse(xhr.response);
            }
...
function traiteXML(XMLDoc, id, template, append) {
    //var data = JSON.parse(XMLDoc);
    var templ = document.getElementById(template).innerHTML;
    var rendered = Mustache.render(templ, XMLDoc);

    if(append == true){
        document.getElementById(id).insertAdjacentHTML('beforeend', rendered);
    }
    else {
